...
const member = {
	...
	async joinValidator(req, res, next){
		...
		* 1. 아이디 자리수 6~20자, 알파벳 + 숫자(정규표현식으로 구현)
		* 2. 비밀번호 자리수 8자리 이상, 알파벳 1개 이상, 특수문자, 숫자 포함되는 복잡성
		* 3. 비밀번호 확인
		...
		*/
		...
		/** 1. 아이디 자리수 6~20자, 알파벳 + 숫자(정규표현식으로 구현) */
			...
			if(memId.length < 6 || memId.length >20 || /[^a-z0-9] /i.test(memId)){
				throw new Error("아이디는 알파벳과 숫자로 구성된 6자리이상 20자리 이하로 입력해 주세요.");
			}
			...
			*/
			if(req.body.cellPhone){
				...
				if(!pattern.test(cellPhone)){ //휴대전화번호 패턴이 아닌 경우
					throw new Error("휴대전환번호 양식이 아닙니다.");
				}
