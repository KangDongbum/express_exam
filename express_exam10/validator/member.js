...
const validator = {
		...
		*/
		...
		try{
			...
			/** 비밀번호 - 자리수(8이상) 
				반드시 알파벳, 숫자, 특수문자가 포함
				/[^a-z]/i -> a-z아닌 문자(숫자, 특수문자가 들어가 있으면 true -> 정확하게 알파벳 존재 여부 X -> 알파벳이 없음에도 true로 유효성 검사 성공
				
				!/[a-z]/i -> a-z인 문자 조건에서 부정
				!/[0-9]/i -> 숫자 포함여부
				!/[~!@#$%^&*]/
			**/ 
	...
	loginValidator(req,res,next){ 
		if(!req.body.memId){ //아이디가 없는경우
			return alert("아이디를 입력해 주세요.", res);
		}
		if(!req.body.memPw){ //비밀번호가 없는경우
